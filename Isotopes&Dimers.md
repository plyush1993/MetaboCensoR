...
> [!IMPORTANT]
>**Since Isotopes and Dimers are determined by graph construction, the default value of `n` is 1 and `n_d` 3.** <br>
> <ins>For isotopes</ins>, `n` = 1 forces the algorithm to find the $1^{st}$ $^{13}C$ isotope before it can group an $M+2$ or $M+3$ peak. This assumption usually works well because isotope peaks detected in LC-MS peak tables are most commonly dominated by the $^{13}C$ isotope ladder. If `n` is set higher, the app might incorrectly cluster two independent molecules that are simply separated by exactly $2.0067$ Da, even if the intermediate $M+1$ peak is missing.<br>
However, depending on the sample type and metabolite class, additional isotope patterns may be relevant. For example, large carbon-rich molecules such as lipids, saponins, flavonoid glycosides, peptides, and lipopeptides may show clear $M+2$ or $M+3$ isotope peaks. In addition, chlorinated or brominated metabolites produce strong isotope peaks near the $M+2$ region.<br>
For these cases, a more permissive isotope search can be performed by increasing the isotope order to `n` = 3 and using a wider m/z tolerance, for example $0.015$ or $0.05$ Da. This setting should be treated as an extended or diagnostic search and interpreted carefully.<br><br>
> <ins>For dimers</ins>, an `n_d` = 3 provides robust 'bridging' between monomer and dimer clusters. Because these half-integer shifts ($1.5, 2.5, 3.5$ Da) are unique and do not overlap with standard isotope patterns, a higher `n_d` value significantly improves the recovery of co-eluting dimer complexes without risking over-connection of the feature graph.
