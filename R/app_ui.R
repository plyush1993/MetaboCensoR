      ...
      sidebarLayout(
        sidebarPanel(
          ...
          div(style = "display: flex; align-items: center; margin-bottom: 12px;",
              div(
                style = "margin-bottom: -15px;",
          materialSwitch("enable_nl2", "Enable neutral loss filter", FALSE, status = "danger")
              ),
              actionButton(
                inputId = "btnlnl",
                label = "?",
              ...
              bsTooltip(
                id = "btnlnl",
                title = "<b>Enable neutral losses filtering</b><br><br>Define polarity<br><br>Uses m/z and RT shifts + correlation threshold to detect neutral losses features and retains ion with the highest m/z in each pair<br><br>Note: By default employs built-in Neutral Losses list (see details in About Tab)",
                placement = "right",
                trigger = "click",
                options = list(container = "body", html = TRUE)
              )
          ...
          div(style = "display: flex; align-items: center; margin-bottom: 12px;",
              div(
                style = "margin-bottom: -15px;",
          materialSwitch("enable_isf2", "Enable in-source fragment filter", FALSE, status = "danger")
              ),
              actionButton(
                inputId = "btnlf",
                label = "?",
              ...
              bsTooltip(
                id = "btnlf",
                title = "<b>Enable in-source fragment filtering</b><br><br>Uses RT shifts + correlation threshold to detect in-source fragment features and retains ion with the highest m/z in each pair<br><br>Note: we recommend to enable `Control intensity ratio` option, that check precursor / fragment intensity ratio<br><br>Note: we recommend, if possible, to enable `Require MS2 fragment match` option, that check if potential fragment is presented in associated precursor MS2 spectra",
                placement = "right",
                trigger = "click",
                options = list(container = "body", html = TRUE)
              )
          ...
          conditionalPanel(
            ...
            div(
                ...
                div(style = "margin-bottom:-15px;",
                  prettyCheckbox(
                    "isf_ms2_check2", "Require MS2 fragment match", FALSE,
                    icon = icon("check"), status = "primary", animation = "jelly")),
                actionButton(
                  "btnisfms2", "?",
                  class = "btn-primary btn-xs",
                  style = "border-radius:50%; width:22px; height:22px; padding:0;
                           line-height:1; font-size:12px; margin-left:8px;")),
              tags$style(HTML("
                .tooltip-inner {
                  text-align: left;
                  font-size: 16px;
              ...
              bsTooltip(
                id = "btnisfms2",
                title = paste0(
                  "<b>MS2-supported in-source fragment matching and filtering</b><br><br>",
                  "Upload the MGF file corresponding to the peak table, then specify the Feature ID field, m/z tolerance, and relative abundance threshold.<br><br>",
                  "Candidate ISF relationships are retained only when the putative fragment m/z (at specified tolerance) is detected in an MS2 spectrum (at specified minimum relative abundance) of the precursor feature.<br><br>",
            ...
            conditionalPanel(
              condition = "input.isf_ms2_check2",
              fileInput("isf_mgf_input", "MGF file", accept = ".mgf"),
              uiOutput("isf_mgf_status_ui"),
              uiOutput("isf_mgf_field_ui"),
              numericInput("isf_ms2_rel2", HTML("MS2 relative intensity &ge; (% base peak)"), value = 1, min = 0, max = 100, step = 0.5),
              numericInput("isf_ms2_mz_tol2", "MS2 fragment m/z tolerance (Da)", value = 0.01, min = 0, step = 0.001)
          )
          ),
          tags$hr(),
          ...
          prettyCheckbox("show_add_table2", "Show adduct groups", FALSE,icon = icon("check"), status = "primary", animation = "jelly"),
          ...
          prettyCheckbox("show_isf_table2", "Show fragment hits", FALSE,icon = icon("check"), status = "primary", animation = "jelly"),
